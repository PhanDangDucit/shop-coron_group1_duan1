<?php
     ...
     */
    function tao_order($user_id, $contact_id) {
        $sql = "insert into orders (user_id, contact_id) 
        values (?, ?)";
        thay_doi_du_lieu($sql, $user_id, $contact_id);
     ...
     */
    function update_du_lieu_khi_order($order_id, $payment_method_id) {
        $sql = "UPDATE orders
        SET order_status_id = 1, order_date = now(), payment_method_id = ?
        WHERE order_id = $order_id";
        thay_doi_du_lieu($sql, $payment_method_id);
